...
cap rm "$table/Tab5_A.xls"
cap rm "$table/Tab5_A.txt"
cap rm "$table/Tab5_B.xls"
cap rm "$table/Tab5_B.txt"

gen nogrant = uagrntn/(uagrntp/100) * (100-uagrntp)/100
foreach x in uagrntn upgrntn ufloann sgrnt_n igrnt_n nogrant{
	cap gen ln`x' = .
	replace ln`x' = ln(`x'+1)
	recode ln`x' . = 0
}

foreach y in nogrant uagrntn upgrntn ufloann sgrnt_n igrnt_n{
	...
	outreg2 using "$table/Tab5_A.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
	
	reghdfe ln`y' LargeWealthyPost if large == 1, a(year unitid) cl(unitid)
	outreg2 using "$table/Tab5_B.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
}

import delimited using "$table/Tab5_B.txt",clear
save "$table/dta/Tab5.dta", replace
import delimited using "$table/Tab5_A.txt",clear
ap using "$table/dta/Tab5.dta"
save "$table/dta/Tab5.dta", replace
...
loc title = "The Impact of Endowment Tax on Student Count by Financial Aid Status"
loc head = "& (1) & (2) & (3) & (4) & (5) & (6) \\  \midrule Outcome Variables: & \multicolumn{7}{c}{Log Student Count} \\ \cmidrule(l){2-8} & No Aid & Any Aid & Pell Grants & Fed. Loan & St./Lo. Grants & Inst. Grants"

texsaveyt 	_all using "$tex/Tab5_temp.tex", replace ///
				title ("`title'") nonames ///
				headerlines("`head'") ///
...
filefilter "$tex/Tab5_temp.tex" "$tex/Tab5.tex", from("&&&&&&") to("") replace
rm "$tex/Tab5_temp.tex"
