...
cap rm "$table/TabA1_A.xls"
cap rm "$table/TabA1_A.txt"
cap rm "$table/TabA1_B.xls"
cap rm "$table/TabA1_B.txt"
...
foreach y in expaux expser expacs expisp exppub exphos expind expoth{
	...
	outreg2 using "$table/TabA1_A.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
	...
	outreg2 using "$table/TabA1_B.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
}

import delimited using "$table/TabA1_B.txt",clear
save "$table/temp/TabA1.dta", replace
import delimited using "$table/TabA1_A.txt",clear
ap using "$table/temp/TabA1.dta"
save "$table/temp/TabA1.dta", replace
...
texsaveyt 	_all using "$tex/TabA1_temp.tex", replace ///
				title ("`title'") nonames ///
				headerlines("`head'") ///
				bottomlines("`bottom'") ///
...
filefilter "$tex/TabA1_temp.tex" "$tex/TabA1.tex", from("&&&&&&&&") to("") replace
rm "$tex/TabA1_temp.tex"
