...
cap rm "$table/TabA2_A.xls"
cap rm "$table/TabA2_A.txt"
cap rm "$table/TabA2_B.xls"
cap rm "$table/TabA2_B.txt"
...
foreach y in revaux revpri revgov revhos revind revedu revinv revoth{
	...
	outreg2 using "$table/TabA2_A.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
	...
	outreg2 using "$table/TabA2_B.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
}

import delimited using "$table/TabA2_B.txt",clear
save "$table/temp/TabA2.dta", replace
import delimited using "$table/TabA2_A.txt",clear
ap using "$table/temp/TabA2.dta"
save "$table/temp/TabA2.dta", replace
...
texsaveyt 	_all using "$tex/TabA2_temp.tex", replace ///
				title ("`title'") nonames ///
				headerlines("`head'") ///
				bottomlines("`bottom'") ///
...
filefilter "$tex/TabA2_temp.tex" "$tex/TabA2.tex", from("&&&&&&&&") to("") replace
rm "$tex/TabA2_temp.tex"
