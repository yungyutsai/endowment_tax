...
cap rm "$table/Tab2_DDD_A.xls"
cap rm "$table/Tab2_DDD_A.txt"
cap rm "$table/Tab2_DDD_B.xls"
cap rm "$table/Tab2_DDD_B.txt"
...
foreach y in exptot expins expres expgrt revtot revtui{
	...
	outreg2 using "$table/Tab2_DDD_A.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
	...
	outreg2 using "$table/Tab2_DDD_B.xls", append dec(3) nocon addtext(baseline,"`m'") alpha(0.001, 0.01, 0.05)
	
}

import delimited using "$table/Tab2_DDD_B.txt",clear
save "$table/temp/Tab2.dta", replace
import delimited using "$table/Tab2_DDD_A.txt",clear
