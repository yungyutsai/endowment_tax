capture program drop rddplot
program define rddplot, eclass
	syntax anything [if] [, c(real 0) p(integer 4) color(string) bondu(string) bondl(string) t(string) t2(string) SAVEname(string) Graph_options(string)]
	...
	** graphing the figure
	twoway	(sc `depvar' `runvar', mc(black)) ///
