*! This ado file is revised from "texsave 1.5 2nov2020 by Julian Reif"
* revised as "threeparttable" structure
* added subcommand of "rh" as row height and "cs" as column space
* added subcommand of "bottomlines" (lines added before "\bottomrule")
* revised the "nofix" function to make it actually does not fix anything
* replaced \tabularnewline as "\\"
* replaced headersep and footnotespace default as nothing
...
* 1.3: booktabs and tabular packages now required. 'H' allowed as location option (for subfig package). Headerlines option now enabled when nonames option is requested. Fixed hlines bug when numlist was unsorted. SW options turned off by default.
...
program define texsaveyt, nclass
	...
	* Error check the location option.  Set default to "h"
	if `"`location'"'!="" {
	...
	* http://mirror.utexas.edu/ctan/macros/latex/contrib/booktabs/booktabs.pdf
	if `"`footnote'"'!=""                          local footnotespace  ""
	...
	* Headerlines (user-specified code)
	if `"`headerlines'"' != "" {
		...
		* Strip out the first \\
		local header_headerlines : subinstr local header_headerlines `" \\ "' ""
		local header_headerlines `"`header_headerlines' \\"'
	}
	...
	* Autonumber - no number in the first column
	if "`autonumber'"!="" {
		...
		local header_autonumber `"`header_autonumber' \\"'
	...
	* Column names (either varlabels or Stata column names) - don't write these out if user specifies -nonames-
	if "`names'"=="" {
		...
		* Strip out the first delimiter
		...
		local header_colnames `"`header_colnames' \\"'
	}
	...
	* Dataset corrections
	if "`nofix'"=="" | "`endash'"=="" | `"`bold'`italics'`underline'`slanted'`smallcaps'`sansserif'`monospace'`emphasis'"'!="" | "`decimalalign'"!="" {
	...
	******
	if "`frag'" == "" {
		...
		file write `fh' "\usepackage{tabular}" _n
	...
	* Headlines option
	...
	file write `fh' "\renewcommand{\arraystretch}{`rh'}" _n //set row high
	file write `fh' "\setlength{\tabcolsep}{`cs'mm}{}" _n //set column space length
	file write `fh' "\begin{threeparttable}" _n
	if `"`size'"'!="" file write `fh' `"{\\`size'"' _n
	if `"`width'"'=="" file write `fh' "\begin{tabular}{`align'}" _n(2)
	if `"`width'"'!="" file write `fh' "\begin{tabular}{`width'}{`align'}" _n(2)
	...
	* Bottomlines option
	if `"`bottomlines'"' != "" {
		file write `fh' `bottomlines' _n
	}
	
	* SW has a bug with \bottomrule that requires you to output an extra \\
	file write `fh' `"\bottomrule `footnotespace'"' _n(2)	

	* Footnote style #1 only done if user specifies width option: this aligns with columns and needs to go before \end{tabular}
	...
	* Footnote style #2: just a simple flush left after the end of the table
	if `"`footnote'"'!="" & `"`footnotewidth'"'=="" {
		...
		file write `fh' `"\begin{singlespace*}"' _n
		file write `fh' `"\scriptsize \textit{Note:} `footnote'"' _n
		file write `fh' `"\end{singlespace*}"' _n
